...
const PROFILE_CATEGORIES = [
  { id: 'basic', label: '基本情報（氏名・生年月日・住所・連絡先）', fields: ['fullname','furigana','birthDate','age','postalCode','address','phone','daytimePhone'], defaultChecked: true },
  ...
  { id: 'selfpr', label: '自己PR・性格・趣味', fields: ['personality','hobbies','volunteer','interests'], defaultChecked: true },
  { id: 'motivation', label: '志望動機', fields: ['motivation'], defaultChecked: false },
  { id: 'essay', label: '作文素材', fields: ['essayValues','essayExperience','essayVision'], defaultChecked: false },
];
...
function formatProfileForMessage(profile, selectedFields) {
  ...
  if (has('daytimePhone')) lines.push(`昼間の連絡先: ${profile.daytimePhone}`);
  ...
  if (has('volunteer')) lines.push(`ボランティア経験: ${profile.volunteer}`);
  if (has('interests')) lines.push(`興味関心: ${profile.interests}`);
  if (has('essayValues')) lines.push(`大切にしている価値観: ${profile.essayValues}`);
  if (has('essayExperience')) lines.push(`具体的な経験: ${profile.essayExperience}`);
  if (has('essayVision')) lines.push(`応募先でのビジョン: ${profile.essayVision}`);
...
// プロフィール引き継ぎモーダル
let pendingStartMode = null; // 'scratch' | 'upload'
...
function startSession(mode, selectedFields) {
  ...
  if (mode === 'scratch') {
    let msg = `こんにちは。私の名前は${userName}です。応募書類の作成をお願いします。`;
    ...
    // アップロードモードの場合、selectedFieldsを一時保存
    pendingProfileFields = selectedFields;
  }
}

let pendingProfileFields = []; // アップロードモードで使用
